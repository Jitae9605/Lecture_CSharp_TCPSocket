	...
	class MainApp
	{
		...
		static void Main(string[] args)
		{
			...
			try
			{
				...
				{
					MSGID = msgID++,
					MSGTYPE = CONSTRAINT.REQ_FILE_SEND,
				...
				using (Stream fileStream = new FileStream(filepath,FileMode.Open))
				{
					...
					int totalRead = 0;
					ushort msgSeq = 0;
					byte fragmented = (fileStream.Length < CHUNK_SIZE) ? CONSTRAINT.NOT_FRAGMENTED : CONSTRAINT.FAGMENTED;
					while(totalRead < fileStream.Length)
					{
						...
						{
							MSGID = msgID,
							MSGTYPE = CONSTRAINT.FILE_SEND_DATA,
							BODYLEN = (uint)fileMsg.Body.GetSize(),
							FRAGMENTED = fragmented,
							LASTMSG = (totalRead < fileStream.Length) ? CONSTRAINT.NOT_LASTING : CONSTRAINT.LASTING,
							SEQ = msgSeq++
						};
